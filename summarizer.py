...
class VideoSummarizer:
    ...
    def get_lstm_importance_scores(self, frame_features):
        """Get importance scores from trained LSTM model"""
    ...
    def combine_scores(self, lstm_scores, query_scores, lstm_weight=0.6, query_weight=0.4):
        """Combine LSTM and query-based scores"""
    ...
    def summarize_video(self, video_path, query_image_path, 
                       num_keyframes=20, frame_interval=5, use_lstm=True):
        """
        Complete video summarization pipeline with trained model
        """
        print("="*60)
        ...
        # Step 4: Get LSTM importance scores (if model is available)
        lstm_scores = None
        if use_lstm and self.lstm_model is not None:
            print(f"\n4. LSTM IMPORTANCE SCORING:")
            # Convert frame_features dict to array
